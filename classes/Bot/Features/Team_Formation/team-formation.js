...
class TeamFormation extends Activity {
  static defaultTeamForm =
    "Team Member(s): \nTeam Background: \nObjective: \nFun Fact About Team: \nLooking For: ";
  static defaultProspectForm =
  ...
  static async createTeamRole(roleManager) {
    winston.loggers
      .get(roleManager.guild.id)
      .verbose(`Team formation team role has been created!`, {
  ...
  static async createProspectRole(roleManager) {
    winston.loggers
      .get(roleManager.guild.id)
      .verbose(`Team formation prospect role has been created!`, {
        event: "Team Formation",
      });
    return await roleManager.create({
      data: {
  ...
  constructor(teamFormationInfo) {
    super({
      ...
      initBotInfo: teamFormationInfo.initBotInfo,
    });

    if (!teamFormationInfo?.teamInfo || !teamFormationInfo?.prospectInfo)
    ...
    if (!teamFormationInfo?.guild)
      throw new Error("A guild is required for a team formation!");
    ...
    this.prospectInfo = {
      ...
      form:
        teamFormationInfo.prospectInfo?.form ||
        TeamFormation.defaultProspectForm,
    ...
    this.isNotificationEnabled =
      teamFormationInfo?.isNotificationsEnabled || false;
    ...
    winston.loggers
      .get(this.guild.id)
      .event(`A Team formation has been created!`, { event: "Team Formation" });
    winston.loggers
      .get(this.guild.id)
      .verbose(`A Team formation has been created!`, {
        event: "Team Formation",
        data: { teamFormationInfo: teamFormationInfo },
      });
  }
  ...
  validatePartyInfo(partyInfo) {
    if (
      !partyInfo?.emoji &&
      typeof partyInfo.emoji != (GuildEmoji || ReactionEmoji)
    )
      throw new Error(
        "A Discord emoji is required for a TeamFormationPartyInfo"
      );
    if (!partyInfo?.role && typeof partyInfo.role != Role)
      throw new Error("A Discord Role is required in a TeamFormationPartyInfo");
    if (partyInfo.signupEmbed && typeof partyInfo.signupEmbed != MessageEmbed)
      throw new Error("The message embed must be a Discord Message Embed");
    if (partyInfo.form && typeof partyInfo.form != "string")
      throw new Error("The form must be a string!");
  }
  ...
  async createChannels() {
    ...
    this.channels.prospectCatalogue = await this.room.addRoomChannel({
      ...
      info: {
        topic:
          "Information about users looking to join teams can be found here. Happy hunting!!!",
      },
      permissions: [
        {
          id: this.initBotInfo.roleIDs.everyoneRole,
    ...
    this.channels.teamCatalogue = await this.room.addRoomChannel({
      ...
      info: {
        topic:
          "Channel for teams to post about themselves and who they are looking for! Expect people to send you private messages.",
      },
      permissions: [
        {
          id: this.initBotInfo.roleIDs.everyoneRole,
    ...
    winston.loggers
      .get(this.guild.id)
      .verbose(`Team formation channels have been created!`, {
        event: "Team Formation",
      });
  }
  ...
  async start(signupEmbedCreator = null) {
    ...
    if (signupEmbedCreator) {
      embed = signupEmbedCreator(
        this.teamInfo.emoji,
        this.prospectInfo.emoji,
        this.isNotificationEnabled
      );
    } else {
      embed = new MessageEmbed()
        .setColor((await firebaseUtil.getInitBotInfo(this.guild.id)).colors.embedColor)
        .setTitle("Team Formation Information")
        .setDescription(
          "Welcome to the team formation section! If you are looking for a team or need a few more members to complete your ultimate group, you are in the right place!"
        )
        .addField(
          "How does this work?",
          "* Once you react to this message, I will send you a template you need to fill out and send back to me via DM. \n* Then I will post your information in the channels below. \n* Then, other members, teams, or yourself can browse these channels and reach out via DM!"
        )
        .addField(
          "Disclaimer!!",
          "By participating in this activity, you consent to other server members sending you a DM."
        )
        .addField(
          "Teams looking for new members",
          "React with " +
            this.teamInfo.emoji.toString() +
            " and the bot will send you instructions."
        )
        .addField(
          "Prospects looking for a team",
          "React with " +
            this.prospectInfo.emoji.toString() +
            " and the bot will send you instructions."
        );
    }
    ...
    winston.loggers
      .get(this.guild.id)
      .event(
        `The team formation has started. ${
          signupEmbedCreator
            ? "A custom embed creator was used"
            : "The default embed was used."
        }`,
        { event: "Team Formation" }
      );
    winston.loggers
      ...
      .verbose(
        ...
        { event: "Team Formation", data: { embed: embed } }
      );

    const signupCollector = signupMsg.createReactionCollector(
      (reaction, user) =>
        !user.bot &&
        (reaction.emoji.name === this.teamInfo.emoji.name ||
          reaction.emoji.name === this.prospectInfo.emoji.name)
    );
    ...
    signupCollector.on("collect", (reaction, user) => {
      ...
      winston.loggers
        .get(this.guild.id)
        .userStats(
          `The user ${user.id} is signing up to the team formation as a ${
  ...
  async reachOutToUser(user, isTeam) {
    ...
    let console = new Console({
      ...
      description:
        "We are very excited for you to find your perfect " +
        (isTeam ? "team members." : "team.") +
    ...
    if (this.isNotificationEnabled)
      console.addField(
        "READ THIS!",
        "As soon as you submit your form, you will be notified of every new " +
          (isTeam ? "available prospect." : "available team.") +
          " Once you close your form, you will stop receiving notifications!"
      );

    await console.addField(
      "Format:",
      isTeam
        ? this.teamInfo.form || TeamFormation.defaultTeamForm
        : this.prospectInfo.form || TeamFormation.defaultProspectForm
    );
    ...
    await console.addFeature(
      Feature.create({
        ...
        description:
          "React to this emoji, wait for my prompt, and send the finished form.",
        emoji: "🇩",
        callback: async (user, reaction, stopInteracting, console) => {
          ...
          try {
            ...
            logger.verbose(
              `I was able to get the user's team formation response: ${catalogueMsg.cleanContent}`,
              { event: "Team Formation" }
            );
          } catch (error) {
            logger.warning(
              `While waiting for a user's team formation response I found an error: ${error}`,
              { event: "Team Formation" }
            );
            user.dmChannel
              .send(
                "You have canceled the prompt. You can try again at any time!"
              )
          ...
          sendEmbedToMember(
            user,
            {
              ...
              description:
                ...
                `Once you find your ${
                  isTeam ? "members" : "ideal team"
                } please react to my original message with ⛔ so I can remove your post. Good luck!!!`,
            },
            15
          );
          logger.event(
            `The user ${user.id} has successfully sent their information to the team formation feature.`,
            { event: "Team Formation" }
          );
          ...
          addRoleToMember(
            ...
            isTeam ? this.teamInfo.role : this.prospectInfo.role
          );
          ...
          await console.addFeature(
            Feature.create({
              ...
              description:
                "React with this emoji if you are done with team formation.",
              emoji: "⛔",
              callback: (user, reaction, stopInteracting, console) => {
                ...
                sendMessageToMember(
                  user,
                  "This is great! You are all set! Have fun with your new team! Your message has been deleted.",
                  true
                );
                ...
                logger.event(
                  `The user ${user.id} has found a team and has been removed from the team formation feature.`,
                  { event: "Team Formation" }
                );
  ...
  async gatherForm(user, isTeam) {
    var formContent = await StringPrompt.single({
      prompt:
        "Please send me your completed form, if you do not follow the form your post will be deleted!",
    ...
    const embed = new MessageEmbed()
      ...
      .setColor(
        isTeam ? this.teamInfo.role.hexColor : this.prospectInfo.role.hexColor
      );

    let channel = isTeam
      ? this.channels.teamCatalogue
      : this.channels.prospectCatalogue;

    let catalogueMsg = await channel.send(
      (this.isNotificationEnabled
        ? "<@&" +
          (isTeam ? this.prospectInfo.role.id : this.teamInfo.role.id) +
          ">, "
        : "") +
        "<@" +
        user.id +
        (isTeam
          ? "> and their team are looking for more team members!"
          : ">  is looking for a team to join!"),
      { embed: embed }
    );
    ...
    winston.loggers
      ...
      .verbose(
        `A message with the user's information has been sent to the channel ${channel.name} with id ${channel.id}.`,
        { event: "Team Formation", data: embed }
      );
