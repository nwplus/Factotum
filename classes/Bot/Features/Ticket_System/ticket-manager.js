...
class TicketManager {
    ...
    async startTicketCreationProcess(user, role, channel) {
        ...
        if (role.members.size <= 0) {
            ...
            winston.loggers.get(this.parent.initBotInfo._id).userStats(`The cave ${this.parent.name} received a ticket from user ${user.id} but was canceled due to no mentor having the role ${role.name}.`, { event: 'Ticket Manager' });
            return;
        }
        ...
        try {
            ...
            winston.loggers.get(this.parent.initBotInfo._id).warning(`New ticket was canceled due to error: ${error}`, { event: 'Ticket Manager' });
            return;
        }
