...
class Prompt {
    ...
    static async messagePrompt({prompt, channel, userId}, responseType, time = 0) {
        ...
        winston.loggers.get(channel.guild.id).event(`The message prompt has been used in channel ${channel.name} for user ${userId}`);
        ...
        try {
            ...
            winston.loggers.get(channel.guild.id).verbose(`Prompt in ${channel.name} with id ${channel.id} for user ${userId} timed out!`);
        ...
        if (msg.content.toLowerCase() === 'cancel') {
            winston.loggers.get(channel.guild.id).verbose(`Prompt in ${channel.name} with id ${channel.id} for user ${userId} was canceled!`);
            let cancelError = new Error("The prompt has been canceled.");
            cancelError.name = 'Cancel'
            throw cancelError;
        }

        winston.loggers.get(channel.guild.id).verbose(`A prompt has been sent to ${channel.name} with id ${channel.id} for user ${userId} of type ${responseType}. Message: ${prompt}. The response was ${msg.cleanContent}.`);
    ...
    static async numberPrompt({prompt, channel, userId}) {
        winston.loggers.get(channel.guild.id).event(`The number prompt has been used in channel ${channel.name} for user ${userId}`);
        ...
        if (invalid) {
            ...
            winston.loggers.get(channel.guild.id).verbose(`The number prompt in channel ${channel.name} for user ${userId} had an invalid response.`);
    ...
    static async yesNoPrompt({prompt, channel, userId}) {
        winston.loggers.get(channel.guild.id).event(`The yes/no prompt has been used in channel ${channel.name} for user ${userId}`);
        ...
        if (promptMsg.content.toLowerCase() === 'no') return false;
            ...
            winston.loggers.get(channel.guild.id).verbose(`The number prompt in channel ${channel.name} for user ${userId} had an invalid response, yes or no was not returned.`);
    ...
    static async channelPrompt({prompt, channel, userId}) {
        winston.loggers.get(channel.guild.id).event(`The channel prompt has been used in channel ${channel.name} for user ${userId}`);
        let promptMsg = await Prompt.messagePrompt({prompt, channel, userId}, 'mention');
        let channels = promptMsg.mentions.channels;
        if (!channels.first()) {
            winston.loggers.get(channel.guild.id).verbose(`The channel prompt in channel ${channel.name} for user ${userId} had an invalid response, no channels were mentioned.`);
    ...
    static async rolePrompt({prompt, channel, userId}) {
        winston.loggers.get(channel.guild.id).event(`The role prompt has been used in channel ${channel.name} for user ${userId}`);
        let promptMsg = await Prompt.messagePrompt({prompt, channel, userId}, 'mention');
        let roles = promptMsg.mentions.roles;
        if (!roles.first()) {
            winston.loggers.get(channel.guild.id).verbose(`The role prompt in channel ${channel.name} for user ${userId} had an invalid response, no roles were mentioned.`);
    ...
    static async memberPrompt({prompt, channel, userId}) {
        winston.loggers.get(channel.guild.id).event(`The member prompt has been used in channel ${channel.name} for user ${userId}`);
        let promptMsg = await Prompt.messagePrompt({prompt, channel, userId}, 'mention');
        let members = promptMsg.mentions.members;
        if (!members.first()) {
            winston.loggers.get(channel.guild.id).verbose(`The member prompt in channel ${channel.name} for user ${userId} had an invalid response, no members were mentioned.`);
