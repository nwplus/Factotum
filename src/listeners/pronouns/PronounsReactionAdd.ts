...
class PronounsReactionAdd extends Listener<typeof Events.MessageReactionAdd> {
  public override async run(reaction: MessageReaction, user: User) {
    ...
    if (member) await member.roles.add(roleOrder[roleIndex]);
  }
}

export default PronounsReactionAdd;
