...
const {
  ...
  saveToLeaderboard,
  retrieveLeaderboard,
} = require("../../db/firebase/firebaseUtil");
...
class DiscordContests extends Command {
  ...
  async chatInputRun(interaction) {
    ...
    let adminConsole = await guild.channels.fetch(
    ...
    if (
      Object.values(initBotInfo.roleIDs).includes(roleId.id) ||
      initBotInfo.verification.roles.some((r) => r.roleId === roleId.id)
    ) {
      interaction.reply({
        content:
          "This role cannot be used! Please pick a role that is specifically for Discord Contest notifications!",
        ephemeral: true,
      });
      return;
    }
    ...
    var string =
      "Discord contests starting soon! Answer questions for a chance to win prizes!";
    ...
    const leaderboard = new MessageEmbed().setTitle("Leaderboard");

    let pinnedMessage = await channel.send({
      content: "<@&" + roleId + ">",
    ...
    const roleSelectionCollector = pinnedMessage.createReactionCollector({
    ...
    interaction.reply({
      content: "Discord contest has been started!",
      ephemeral: true,
    });
    const controlPanel = await adminConsole.send({
      content: "Discord contests control panel. Status: Active",
      components: [row],
    });
    ...
    collector.on("collect", async (i) => {
      if (i.customId == "refresh") {
        ...
        paused = true;
        await i.reply({
          content: "Discord contests has been paused!",
          ephemeral: true,
        });
        await controlPanel.edit({
          content: "Discord contests control panel. Status: Paused",
        });
        ...
        paused = false;
        await i.reply({
          content: "Discord contests has been un-paused!",
          ephemeral: true,
        });
        await controlPanel.edit({
          content: "Discord contests control panel. Status: Active",
        ...
        await i.reply({
          content: "Wrong button or wrong permissions!",
          ephemeral: true,
        });
      }
    });
    ...
    async function updateLeaderboard(memberId) {
      ...
      const newLeaderboard = new MessageEmbed(leaderboard).setDescription(
        leaderboardString,
      );
    ...
    async function sendQuestion(initBotInfo) {
      ...
      if (!answers?.length) {
        ...
        const questionMsg = await adminConsole.send({
          content:
            "<@&" + initBotInfo.roleIDs.staffRole + ">" + "need manual review!",
          embeds: [qEmbed],
          components: [row],
        });
        ...
        const filter = (i) =>
          ...
          (guild.members.cache
            ...
            guild.members.cache
              .get(i.user.id)
              .roles.cache.has(initBotInfo.roleIDs.adminRole));
        const collector = await questionMsg.createMessageComponentCollector({
          filter,
        });

        collector.on("collect", async (i) => {
          const winnerRequest = await i.reply({
            content:
              "<@" + i.user.id + "> Mention the winner in your next message!",
            fetchReply: true,
          });

          const winnerFilter = (message) => message.author.id === i.user.id; // error?
          const winnerCollector = await adminConsole.createMessageCollector({
            filter: winnerFilter,
            max: 1,
          });
          winnerCollector.on("collect", async (m) => {
            if (m.mentions.members.size > 0) {
              ...
              let errorMsg = await i.editReply({
                content: "Message does not include a user mention!",
              });
              setTimeout(function () {
                errorMsg.delete();
              }, 5000);
        ...
        const filter = (m) =>
          !m.author.bot &&
          (initBotInfo.verification.isEnabled
            ? checkForRole(
                m.member,
                initBotInfo.verification.roles.find((r) => r.name === "hacker")
                  ?.roleId,
              )
            : checkForRole(m.member, initBotInfo.roleIDs.memberRole));
        const collector = channel.createMessageCollector({
          filter,
          time: timeInterval * 0.75,
        });
        ...
        collector.on("collect", async (m) => {
          if (!needAllAnswers) {
            ...
            if (!isNaN(answers[0])) {
              if (
                answers.some((correctAnswer) => m.content === correctAnswer)
              ) {
                await channel.send(
                  "Congrats <@" +
                    m.author.id +
                    "> for getting the correct answer! The answer key is " +
                    answers.join(" or ") +
                    ".",
                );
              ...
              answers.some((correctAnswer) =>
                m.content.toLowerCase().includes(correctAnswer.toLowerCase()),
              )
            ) {
