...
class NewCoffeeChats extends PermissionCommand {
  constructor(client) {
    super(
      client,
      {
        ...
        description:
          "Will initialize the coffee chat functionality for the given workshop.",
        guildOnly: true,
        args: [
          {
            key: "activityName",
  ...
  async runCommand(botGuild, message, { activityName, numOfGroups }) {
    ...
    let coffeeChats = await new CoffeeChats(
      {
      ...
      numOfGroups,
    ).init(message.channel, message.author.id);

    // report success of coffee chat creation
    replyAndDelete(
      message,
      "Activity named: " + activityName + " now has coffee chat functionality.",
    );
  }
}
