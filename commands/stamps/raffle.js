...
class Raffle extends PermissionCommand {
  constructor(client) {
    super(
      client,
      {
        ...
        description:
          "parses each hacker for their stamps and draws winners from them, one entry per stamp",
        guildOnly: true,
        args: [
          {
            key: "numberOfWinners",
  ...
  async runCommand(initBotInfo, message, { numberOfWinners }) {
    ...
    let validMembers = message.guild.members.cache.filter((member) =>
      member.roles.cache.has(initBotInfo.roleIDs.memberRole),
    );
    var memberCount = validMembers.size;
    if (memberCount <= numberOfWinners) {
      message.channel
        .send("Whoa there, you want more winners than hackers!")
        .then((msg) => {
    ...
    validMembers.forEach((member) => {
      let roleId = member.roles.cache.find((role) =>
        initBotInfo.stamps.stampRoleIDs.has(role.id),
      );
      if (!roleId) return;
