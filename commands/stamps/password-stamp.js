...
class PasswordStamp extends PermissionCommand {
  constructor(client) {
    super(
      client,
      {
        ...
        description:
          "gives a stamp to everyone who reacted and gave the correct password",
        args: [
          {
            key: "activityName",
  ...
  async runCommand(initBotInfo, message, { activityName, password, stopTime }) {
    ...
    try {
      if (activityName === "") {
        activityName = await StringPrompt.single({
          prompt: "Please respond with the workshop/activity name.",
          channel,
          userId,
          cancelable: true,
        });
      ...
      if (password === "") {
        password = await StringPrompt.single({
          prompt:
            "Please respond with the password for hackers to use to get stamp.",
          channel,
          userId,
          cancelable: true,
        });
      ...
      var targetChannel = await ChannelPrompt.single({
        prompt:
          "What channel do you want to send the stamp collector to? Users should have access to this channel!",
        channel,
        userId,
        cancelable: true,
      });
    ...
    const qEmbed = new MessageEmbed()
      ...
      .setTitle(
        "React with anything to claim a stamp for attending " + activityName,
      )
      .setDescription(
        "Once you react to this message, check for a DM from this bot. **You can only emoji this message once!**",
      )
      .addField(
        "A Password Is Required!",
        "Through the Bot's DM, you will have 3 attempts in the first 60 seconds to enter the correct password.",
      );
    ...
    targetChannel.send(qEmbed).then((msg) => {
      ...
      const emojiFilter = (reaction, user) =>
        !user.bot && !seenUsers.has(user.id);
      const collector = msg.createReactionCollector(emojiFilter, {
        time: 1000 * stopTime * 60,
      }); // stopTime is in minutes, multiply to get seconds, then milliseconds
      ...
      collector.on("collect", async (reaction, user) => {
        ...
        var dmMessage = await sendEmbedToMember(user, {
          description:
            "You have 60 seconds and 3 attempts to type the password correctly to get the " +
            activityName +
            " stamp.\n" +
        ...
        const pwdCollector = dmMessage.channel.createMessageCollector(filter, {
          time: 60000,
          max: 3,
        });
          ...
          if (!correctPassword) {
            if (incorrectPasswords < 3) {
              sendEmbedToMember(user, {
                ...
                description:
                  "Time's up! You took too long to enter the password for the " +
                  activityName +
                  " stamp. If you have extenuating circumstances please contact an organizer.",
              });
            } else {
              sendEmbedToMember(user, {
                title: "Stamp Collector",
                description:
                  "Incorrect. You have no attempts left for the " +
                  activityName +
                  " stamp. If you have extenuating circumstances please contact an organizer.",
              });
            }
      ...
      collector.on("end", (collected) => {
        if (
          msg.guild.channels.cache.find(
            (channel) => channel.name === targetChannel.name,
          )
        ) {
          msg.edit(
            qEmbed.setTitle(
              "Time's up! No more responses are being collected. Thanks for participating in " +
                activityName +
                "'s booth!",
            ),
          );
