...
class InitBot extends Command {
  ...
  registerApplicationCommands(registry) {
    registry.registerChatInputCommand((builder) =>
      builder
        ...
        .addChannelOption((option) =>
          option
            .setName("welcome_support_channel")
            .setDescription(
              "Mention the channel for verification issues (must be viewable to guests)!",
            )
            .setRequired(true),
        )
        .addAttachmentOption((option) =>
          option
            ...
            .setDescription(
              "File: array of objects! Each role string in the participants' database and corresponding role ID.",
            )
            .setRequired(true),
        )
        // .addBooleanOption(option =>
  ...
  async chatInputRun(interaction) {
    ...
    const hackathonName =
      interaction.options.getString("hackathon_name") ||
    ...
    const duplicateQuery = await initBotInfoRef
      .where("hackathonName", "==", hackathonName)
      .get();
    ...
    if (useVerification) {
      ...
      welcomeSupportChannel = interaction.options.getChannel(
        "welcome_support_channel",
      ).id;
      verificationRoles =
        interaction.options.getAttachment("verification_roles");
      try {
        ...
        await interaction.followUp({
          content:
            "An error occurred with the file upload or verification roles upload!",
          ephemeral: true,
        });
        return;
      }
    ...
    // }
    const embedColor =
      interaction.options.getString("embed_colour") || "#26fff4";

    // ask the user to move our role up the list
    await interaction.followUp({
      content:
        "Before we move on, could you please move my role up the role list as high as possible, this will give me the ability to assign roles!",
      ephemeral: true,
    });
  ...
  async getValidVerificationTypes(channel, userId, guestRole, memberRole) {
    let typeMsg = await MessagePrompt.prompt({
      prompt: `Please tell me the type and mention the role for a verification option. 
            For example: hacker @hacker . Make sure you add nothing more to the message!`,
      channel,
      userId,
    });
    ...
    if (role.id === guestRole.id || role.id === memberRole.id) {
      sendMsgToChannel(
        channel,
        userId,
        "Guest and member roles cannot be used for verification. " +
          "Please try again.",
        30,
      );
      return await this.getValidVerificationTypes(
        channel,
        userId,
        guestRole,
        memberRole,
