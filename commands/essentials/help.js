...
class Help extends Command {
  ...
  async run(message) {
    ...
    const textEmbed = new MessageEmbed()
      ...
      .setDescription(
        "All other interactions with me will be via emoji reactions!",
      )
      .setTimestamp();
    ...
    for (var i = 0; i < length; i++) {
      ...
      if (command.format != null) {
        textEmbed.addField(
          this.client.commandPrefix + command.name,
          command.description + ", arguments: " + command.format,
        );
      } else {
        textEmbed.addField(
          this.client.commandPrefix + command.name,
          command.description + ", no arguments",
        );
      }
    }
