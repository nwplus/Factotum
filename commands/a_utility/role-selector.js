...
class RoleSelector extends PermissionCommand {
  constructor(context, options) {
    super(
      context,
      {
        ...
        name: "role-selector",
        description:
          "Will let users transfer roles. Useful for sponsor reps that are also mentors!",
      },
      {
        role: PermissionCommand.FLAGS.STAFF_ROLE,
        roleMessage:
          "Hey there, the command !role-selector is only available to staff!",
      },
    );
  }
  ...
  async chatInputRun(interaction) {
    ...
    if (roleSelectorData?.emojis) {
      for (const [emoji, name] of Object.entries(roleSelectorData.emojis)) {
...
async function makeRoleSelectionEmbed() {
  ...
  if (emojisMap.size > 0) {
    roleSelectionEmbed.setFields(
      Array.from(emojisMap).map(([k, v]) => ({
        name: k,
        value: `${v.title} - ${v.description}`,
      })),
    );
  }
 ...
 */
async function listenToRoleSelectorReaction(initBotInfo, guild, messageEmbed) {
  ...
  reactionCollector.on("collect", async (reaction, user) => {
    ...
    if (reaction.emoji.name === newTransferEmoji) {
      ...
      const roleSelectListener =
        roleSelectMenu.createMessageComponentCollector();
      roleSelectListener.on("collect", async (i) => {
        ...
        if (i.customId === "transfer_role") {
          ...
          if (submitted) {
            ...
            const description =
              submitted.fields.getTextInputValue("transfer_desc");

            await submitted.reply(
              "New role selector details successfully submitted!",
            );

            const askForEmoji = await user.send(
              "React to this message with the emoji for the role!",
            );
            ...
            emojiCollector.on("collect", async (reaction, user) => {
              ...
              const roleSelectorData = (
                await savedMessagesCol.doc("role-selector").get()
              ).data();
              if (
                roleSelectorData?.emojis &&
                reaction.emoji.name in roleSelectorData.emojis
              ) {
                user
                  .send(
                    "Emoji is already used in another role. Please react again.",
                  )
                  .then((msg) => {
                ...
                emojisMap.set(reaction.emoji.name, {
                  title,
                  description,
                  roleId: role.id,
                });
                ...
                messageEmbed.edit({
                  embeds: [new MessageEmbed(await makeRoleSelectionEmbed())],
                });
