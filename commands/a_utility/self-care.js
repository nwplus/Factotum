...
class SelfCareReminders extends Command {
    ...
    registry.registerChatInputCommand((builder) =>
      builder
        ...
        .addBooleanOption((option) =>
          option
            .setName("start_reminder_now")
            .setDescription(
              "True to start first reminder now, false to start it after one interval",
            )
            .setRequired(false),
        ),
    ),
  ...
  async chatInputRun(interaction) {
    ...
    const startEmbed = new MessageEmbed()
      ...
      .setTitle(
        "To encourage healthy hackathon habits, we will be sending hourly self-care reminders!",
      );

    interaction.reply({
      content: "Self-care reminders started!",
      ephemeral: true,
    });
    ...
    roleId
      ...
      : interaction.channel.send({ embeds: [startEmbed] });
    ...
    const controlPanel = await adminConsole.send({
      content: "Self care reminders started by <@" + userId + ">",
      components: [row],
    });
    const isAdminOrStaffFilter = (i) =>
      !i.user.bot &&
      (guild.members.cache
        .get(i.user.id)
        .roles.cache.has(initBotInfo.roleIDs.staffRole) ||
        guild.members.cache
          .get(i.user.id)
          .roles.cache.has(initBotInfo.roleIDs.adminRole));
    const collector = controlPanel.createMessageComponentCollector({
      filter: isAdminOrStaffFilter,
    });
    collector.on("collect", async (i) => {
      if (interval != null && !paused && i.customId == "pause") {
        ...
        discordLog(
          guild,
          "Self care reminders paused by <@" + i.user.id + ">!",
        );
        await i.reply({
          content: "Self care reminders has been paused!",
          ephemeral: true,
        });
        ...
        discordLog(
          guild,
          "Self care reminders restarted by <@" + i.user.id + ">!",
        );
        await i.reply({
          content: "Self care reminders has been un-paused!",
          ephemeral: true,
        });
      } else {
        await i.reply({
      ...
      if (data === null) {
        discordLog(
          ...
          "<@&" +
            ...
            "> HI, PLEASE FEED ME more self-care messages!!",
        );
        clearInterval(interval);
        return;
      }
