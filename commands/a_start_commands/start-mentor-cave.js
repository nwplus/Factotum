...
class StartMentorCave extends Command {
        ...
        builder
          ...
          .addIntegerOption((option) =>
            option
              ...
              .setDescription(
                "How long (minutes) before bot asks users to delete ticket channels",
              )
              .setRequired(true),
          )
          .addIntegerOption((option) =>
            option
              ...
              .setDescription(
                "How long (minutes) shall a ticket go unaccepted before the bot sends a reminder to all mentors?",
              )
              .setRequired(true),
          )
          ...
          .addChannelOption((option) =>
            option
              .setName("mentor_role_selection_channel")
              .setDescription(
                "Tag the channel where mentors can select their specialties",
              )
              .setRequired(false),
          )
          .addChannelOption((option) =>
            option
              ...
              .setDescription(
                "Tag the channel where mentor tickets will be sent",
              )
              .setRequired(false),
          )
          .addChannelOption((option) =>
            option
              .setName("request_ticket_channel")
              .setDescription(
                "Tag the channel where hackers can request tickets",
              )
              .setRequired(false),
          ),
  ...
  async chatInputRun(interaction) {
    try {
      ...
      const reminderTime = interaction.options.getInteger(
      ...
      const mentorRoleSelectionChannel =
        interaction.options.getChannel("mentor_role_selection_channel") ??
        (mentorRoleSelectionChannelId
          ? await guild.channels.fetch(mentorRoleSelectionChannelId)
          : null);

      const incomingTicketsChannel =
        interaction.options.getChannel("incoming_tickets_channel") ??
        (incomingTicketsChannelId
          ? await guild.channels.fetch(incomingTicketsChannelId)
          : null);

      const requestTicketChannel =
        interaction.options.getChannel("request_ticket_channel") ??
      ...
      if (
        ...
        await interaction.reply({
          content: "Please enter all 3 channels!",
          ephemeral: true,
        });
        return;
      }
      ...
      const mentorRoleColour = guild.roles.cache.find(
        (role) => role.id === this.initBotInfo.roleIDs.mentorRole,
      ).hexColor;
      for (let value of emojisMap.values()) {
        const findRole = guild.roles.cache.find(
          (role) => role.name.toLowerCase() === `M-${value}`.toLowerCase(),
        );
        if (!findRole) {
      ...
      /** @type {Message} */
      const roleSelectionMsg = await mentorRoleSelectionChannel.send({
        embeds: [roleSelection],
      });
      ...
      const mentorCaveDoc = firebaseUtil
        .getSavedMessagesSubCol(guild.id)
        .doc("mentor-cave");
      ...
      /** @type {Message} */
      const requestTicketConsole = await requestTicketChannel.send({
        embeds: [requestTicketEmbed],
      ...
      const adminConsole = await guild.channels.fetch(
        this.initBotInfo.channelIDs.adminConsole,
      );

      /** @type {Message} */
      const adminControls = await adminConsole.send({
        embeds: [adminEmbed],
        components: [adminRow],
      });
  ...
  async startChannelActivityListener(
    ...
    if (ticketText.parentId && ticketVoice.parentId) {
      const activityListener = ticketText.createMessageCollector({
        filter: (m) => !m.author.bot,
        idle: inactivePeriod * 60 * 1000,
      });
      activityListener.on("end", async (collected) => {
        if (!ticketText.parentId || !ticketVoice.parentId) return;
        if (
          collected.size === 0 &&
          ticketVoice.members.size === 0 &&
          ticketMsg.embeds[0].color != "#90EE90"
        ) {
          const remainingMembers = await ticketCategory.members
            .filter(
              (member) =>
                !member.roles.cache.has(this.initBotInfo.roleIDs.adminRole) &&
                !member.user.bot,
            )
          ...
          const msgText =
            ...
            '> Hello! I detected some inactivity in this channel. If you are done and would like to leave this ticket, please go to the pinned message and click the "Leave" button. If you would like to keep this channel a little longer, please click the button below.\n**If no action is taken in the next ' +
            bufferTime +
            " minutes, the channels for this ticket will be deleted automatically.**";
          ...
          const warning = await ticketText.send({
            content: msgText,
            components: [row],
          });
          ...
          warning
            .awaitMessageComponent({
              filter: (i) => !i.user.bot,
              time: bufferTime * 60 * 1000,
            })
            .then((interaction) => {
              interaction.reply(
                "You have indicated that you need more time. I'll check in with you later!",
              );
            ...
            .catch(() => {
              if (
                !ticketText.parentId ||
                !ticketVoice.parentId ||
                ticketMsg.embeds[0].color == "#90EE90"
              )
                return;
              if (ticketVoice.members.size === 0) {
                this.deleteTicketChannels(
                  ticketText,
                  ticketVoice,
                  ticketCategory,
                  ticketMsg,
                  ticketMsg.embeds[0]
                    ...
                    .addFields([
                      {
                        name: "Ticket closed",
                        value: "Deleted due to inactivity",
                      },
                    ]),
                );
              } else {
  ...
  async tryRestoreReactionListeners(guild) {
    ...
    if (!mentorCaveDoc.exists)
      return "Saved messages doc for mentor cave does not exist";
    ...
    const { messageId: reactionMessageId, channelId: reactionChannelId } =
    ...
    const { channelId: controlsChannelId, messageId: controlsMessageId } =
    ...
    try {
      ...
      const adminControls =
        await adminControlsChannel.messages.fetch(controlsMessageId);
      const adminConsole = await guild.channels.fetch(
        initBotInfo.channelIDs.adminConsole,
      );
      listenToAdminControls(
        initBotInfo,
        guild,
...
function makeRoleSelectionEmbed() {
  ...
  const roleSelection = new MessageEmbed()
    .setTitle(
      "Choose what you would like to help hackers with! You can un-react to deselect a role.",
    )
    .setDescription(
...
function makeSelectMenuRow() {
  ...
  const selectMenuRow = new MessageActionRow().addComponents(
    new MessageSelectMenu().setCustomId("ticketType").addOptions(options),
  );
...
async function listenToRoleReactions(guild, roleSelectionMsg) {
  const collector = roleSelectionMsg.createReactionCollector({
  ...
  collector.on("remove", async (reaction, user) => {
    if (emojisMap.has(reaction.emoji.name)) {
      ...
      const value = emojisMap.get(reaction.emoji.name);
      const findRole = member.roles.cache.find(
        (role) => role.name.toLowerCase() === `M-${value}`.toLowerCase(),
      );
      if (findRole)
...
function listenToRequestConsole(
  ...
  const selectMenuCollector =
    requestTicketConsole.createMessageComponentCollector({
  ...
  selectMenuCollector.on("collect", async (i) => {
    if (i.customId === "ticketType") {
      ...
      if (!guild.members.cache.get(i.user.id).roles.cache.has(publicRole.id)) {
        await i.reply({
          content: "You do not have permissions to request tickets!",
          ephemeral: true,
        });
        return;
      }
      const modal = new Modal()
        ...
        .setTitle(
          i.values[0] === "None of the above"
            ? "Request a general mentor ticket"
            : "Request a ticket for " + i.values[0],
        )
        .addComponents([
      ...
      if (submitted) {
        const role =
          i.values[0] === "None of the above"
            ...
            : guild.roles.cache.find(
                (role) =>
                  role.name.toLowerCase() === `M-${i.values[0]}`.toLowerCase(),
              ).id;
        const description =
          submitted.fields.getTextInputValue("ticketDescription");
        ...
        const mentorCaveDoc = firebaseUtil
          ...
          .doc("mentor-cave");
        const ticketNumber =
          (await mentorCaveDoc.get()).data()?.ticketCount ?? 1;
        await mentorCaveDoc.set(
        ...
        const ticketAcceptanceRow = new MessageActionRow().addComponents(
          new MessageButton()
        ...
        const ticketMsg = await incomingTicketsChannel.send({
          content:
            "<@&" + role + ">, requested by <@" + submitted.user.id + ">",
          embeds: [newTicketEmbed],
          components: [ticketAcceptanceRow],
        });
        submitted.reply({
        ...
        const ticketReminder = setTimeout(() => {
          ticketMsg.reply(
            "<@&" + role + "> ticket " + ticketNumber + " still needs help!",
          );
        }, reminderTime * 60000);
        ...
        const deleteTicketRow = new MessageActionRow().addComponents(
          new MessageButton()
        ...
        const deleteTicketCollector =
          ticketReceipt.createMessageComponentCollector({
            filter: (i) => !i.user.bot,
            max: 1,
          });
          ...
          await ticketMsg.edit({
            embeds: [
              ticketMsg.embeds[0]
                .setColor("#FFCCCB")
                .addFields([
                  { name: "Ticket closed", value: "Deleted by hacker" },
                ]),
            ],
            components: [],
          });
        ...
        const ticketAcceptanceCollector =
          ticketMsg.createMessageComponentCollector({
            filter: ticketAcceptFilter,
          });
        ticketAcceptanceCollector.on("collect", async (acceptInteraction) => {
          const inProgressTicketEmbed = ticketMsg.embeds[0]
            .setColor("#0096FF")
            .addFields([
              {
                name: "Helped by:",
          ...
          if (
            acceptInteraction.customId === "acceptIrl" ||
            acceptInteraction.customId === "acceptOnline"
          ) {
...
function listenToAdminControls(
  ...
  const mentorRoleColour = guild.roles.cache.find(
    (role) => role.id === initBotInfo.roleIDs.mentorRole,
  ).hexColor;
  const adminCollector = adminControls.createMessageComponentCollector({
    filter: (i) =>
      !i.user.bot && i.member.roles.cache.has(initBotInfo.roleIDs.adminRole),
  });
  adminCollector.on("collect", async (adminInteraction) => {
    if (adminInteraction.customId === "addRole") {
      const askForRoleName = await adminInteraction.reply({
        content: `<@${adminInteraction.user.id}> name of role to add? Type "cancel" to cancel this operation.`,
        fetchReply: true,
      });
      const roleNameCollector = adminConsole.createMessageCollector({
        filter: (m) => m.author.id === adminInteraction.user.id,
        max: 1,
      });
      let roleName;
      roleNameCollector.on("collect", async (collected) => {
        if (collected.content.toLowerCase() != "cancel") {
          ...
          const askForEmoji = await adminConsole.send(
            `<@${adminInteraction.user.id}> React to this message with the emoji for the role!`,
          );
          const emojiCollector = askForEmoji.createReactionCollector({
            filter: (reaction, user) => user.id === adminInteraction.user.id,
          });
          emojiCollector.on("collect", (collected) => {
            if (emojisMap.has(collected.emoji.name)) {
              adminConsole
                .send(
                  `<@${adminInteraction.user.id}> Emoji is already used in another role. Please react again.`,
                )
                .then((msg) => {
              ...
              adminConsole
                .send(`<@${adminInteraction.user.id}> Role added!`)
                .then((msg) => {
                  setTimeout(() => msg.delete(), 5000);
                });
              roleSelectionMsg.edit({
                embeds: [new MessageEmbed(makeRoleSelectionEmbed())],
              });
              roleSelectionMsg.react(collected.emoji.name);

              const selectMenuOptions =
                makeSelectMenuRow().components[0].options;
              const newSelectMenuRow = new MessageActionRow().addComponents(
