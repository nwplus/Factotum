...
class StartReport extends Command {
  constructor(context, options) {
    super(context, {
      ...options,
      description:
        "Will send report format to user via DM for user to send back via DM. Admins will get the report!",
    });
  }
  ...
  async listenToReports(guild, msg) {
    const checkInCollector = msg.createMessageComponentCollector({
      filter: (i) => !i.user.bot,
    });
    ...
    checkInCollector.on("collect", async (i) => {
      ...
      if (submitted) {
        ...
        try {
          discordLog(
            guild,
            `<@&${guild.roleIDs.staffRole}> New anonymous report:\n\n ${issueMessage}`,
          );
        } catch {
