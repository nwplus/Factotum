...
class GetEmails extends Command {
  constructor(context, options) {
    super(context, {
      ...options,
      description:
        "Check the email of a given user if they have it linked to their Discord ID in our database.",
    });
  }

  registerApplicationCommands(registry) {
    registry.registerContextMenuCommand((builder) =>
      builder.setName(this.name).setType(2),
    );
  }
  async contextMenuRun(interaction) {
    ...
    if (email) {
      ...
      botSpamChannel.send(
        "<@" + interaction.targetUser.id + ">'s email is: " + email,
      );
      return;
    } else {
      interaction.reply({
      ...
      botSpamChannel.send(
        "<@" + interaction.targetUser.id + ">'s email is not in our database!",
      );
      return;
    }
  }
