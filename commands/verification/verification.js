    ...
    async run(message, { email }) {
        ...
        if (message.channel.name === 'welcome') {
            ...
            if ((await discordServices.checkForRole(message.member, discordServices.guestRole)) === true) {
                ...
                'already more then a guest. No need to do it agian!');
            }
        } else {
            discordServices.sendMessageToMember(message.member, 'Hi, the !verify command is only available in the welcome channel!');
        }
    }
