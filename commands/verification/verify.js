  ...
  constructor(client) {
    super(
      client,
      {
        ...
        description:
          "Will verify a guest to its correct role if their email is in our database.",
        args: [
          {
            key: "email",
    ...
    if (!checkForRole(member, initBotInfo.roleIDs.guestRole)) {
      sendEmbedToMember(
        member,
        {
          title: "Verify Error",
          description:
            "You do not need to verify, you are already more than a guest!",
        },
        true,
      );
      return;
